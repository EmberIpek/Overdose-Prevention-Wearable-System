...
def bandpass_filter(samples, order=4, low=0.5, high=3, fs=float):
	# clamp range if sampling below nyquist rate
	if(freq < (2 * high)):
		high = (freq/2) * 0.99
		print("****************FREQ = ", freq, "*****************")
...
def plot_AC(filtered_red_AC, filtered_ir_AC, freq):
...
def get_heartrate():
    # use measured ADC value for height
    # should have documented distance reason...
	peak_indices, _ = signal.find_peaks(filtered_red_AC, height=(None,200), distance=int(freq * 0.4))
	window = 5
	peak_times = np.array(time_sent)[peak_indices]
	heartrate_list = []
...
while True:
	...
	try:
		data = struct.unpack(">fiiiH", packet)
		temperature = data[0]
		red = data[1]
		ir = data[2]
		# time.ticks_ms sent in ms, convert to s
		time_sent.append(data[3] / 1000)
		# time_sent.append(time_received)
		checksum = data[4]
		
		# receive first 100 packets then graph
		if(count < 100):
			count += 1
			...
			# compute average sampling rate using time sent
			dt = np.diff(time_sent)
			# TO DO: clear arrays and recalculate if time_sent too high (sampling period too high)
			freq = 1 / np.mean(dt)
			...
			# plot_AC(filtered_red_AC, filtered_ir_AC, freq)
	except struct.error:
